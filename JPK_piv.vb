Private Sub jpk_gen(co_name As String, co_nip As String, co_mail As String)
...
required = Array("Sales tax code", "Invoice", "Address", "Tax exempt number", _
            "Customer/Vendor", "Document date", "Date", "NET NBP", "VAT NBP")
...
company_info = Array("JPK_VAT", "JPK_VAT (3)", "1-1", "3", "0", "", "", "", "AX Microsoft Dynamics 2009")
company_info2 = Array(co_nip, co_name, co_mail)
...
'\\\\\\\\\\\\\\\ EVERYWERE IN CODE DATA_RANGE HAS "i+1" ITERATION VALUE BECAUSE OF HEADINGS IN FIRST ELEMENT \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    ...
    If purch_mappings Then
         ...
         purcharses(purcharses_counter, 6) = data_range(input_row + 1, document_date)
         purcharses(purcharses_counter, 7) = data_range(input_row + 1, transaction_date)
         For k = 8 To 15
...
Worksheets(jpk_sheet).Range("r1:s1").EntireColumn.NumberFormat = "yyyy-mm-dd"
Worksheets(jpk_sheet).Range("t1:aw1").EntireColumn.NumberFormat = "0.00"
Worksheets(jpk_sheet).Range("ay1").EntireColumn.NumberFormat = "0.00"
Worksheets(jpk_sheet).Range("be1:bf1").EntireColumn.NumberFormat = "yyyy-mm-dd"
Worksheets(jpk_sheet).Range("bg1:bn1").EntireColumn.NumberFormat = "0.00"
Worksheets(jpk_sheet).Range("bp1").EntireColumn.NumberFormat = "0.00"
Worksheets(jpk_sheet).columns("a:ca").AutoFit
Worksheets(jpk_sheet).columns("a:q").NumberFormat = "@"
Worksheets(jpk_sheet).columns("az:bd").NumberFormat = "@"
...
answer = MsgBox("Do You want to export sheet to CSV file?", vbYesNo + vbQuestion, "Export to CSV")
...
Dim FileNAME As String
    ...
    Dim msg As String
    msg = "Sheet has been exported as """ & FileNAME & """"
    MsgBox (msg)
    Application.DisplayAlerts = False
    ThisWorkbook.Saved = True
    Application.Quit
